...
letter_match = Blueprint('letter_match', __name__)

@letter_match.route("/create", methods=["POST"])
def create_letter_match():
    """
    Create a Letter Match game.
    Request JSON looks like:
    {
      "room": "MyLocalRoom",
      "game_type": "LetterMatchSingle" or "LetterMatchLocal" or "LetterMatchOnline",
      "player_names": ["Alice", "Bob"]   (only for local)
      "creator_username": "RealUser"     (only for online and single)
    ...
    if game_type == "LetterMatchSingle":
        if not creator_username:
            return jsonify({"error": "creator_username is required for single play games"}), 400
    ...
    if game_type == "LetterMatchOnline":
        if not creator_username:
...
def join_game():
    """
    Online players can join an existing (not started) LetterMatchOnline game.
    {
      "room": "MyOnlineRoom",
...
def start_game():
    ...
    2. Save start_time
    3. Randomly choose a letter or have user pick (single player)
